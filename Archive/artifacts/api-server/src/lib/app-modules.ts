...
export const SEED_MODULES: Array<{
  ...
  },
  {
    ...
    description: "Enables the microphone button and text-to-speech replies in the AI Assistant chatbot. Users can speak queries and hear responses read aloud. Disable to remove all voice controls from the interface.",
    isEnabled: true,
    isCore: false,
    sortOrder: 50,
  },
...
export async function getEnabledModules(): Promise<Record<string, boolean>> {
  try {
    ...
    return { crm_sales: true, quotes: true, orders: true, contracts: true, voice_commands: true };
  }
}
