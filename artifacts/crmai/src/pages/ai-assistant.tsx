              ...
              <CardContent className="p-6 space-y-5">
                ...
                {highValueOpps.length === 0 ? (
                  ...
                  highValueOpps.map((opp, i) => (
                    <div key={i} className={`rounded-xl p-4 bg-black/40 relative overflow-hidden border ${i === 0 ? "border-primary/30" : "border-white/10"}`}>
                      {i === 0 && <div className="absolute top-0 left-0 w-1 h-full bg-gradient-to-b from-primary to-accent" />}
                      <div className="flex gap-4">
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${i === 0 ? "bg-accent/20 text-accent" : "bg-blue-500/20 text-blue-400"}`}>
                          <User className="w-5 h-5" />
                        </div>
                        <div className="flex-1">
                          <h4 className="font-medium text-white">Follow up with {opp.name}</h4>
                ...
                <div className="border border-white/10 rounded-xl p-4 bg-black/20 mt-2">
                  <div className="flex gap-4">
                    <div className="w-10 h-10 rounded-full bg-green-500/20 flex items-center justify-center shrink-0 text-green-400">
                      <TrendingUp className="w-5 h-5" />
                    </div>
                    <div>
                      <h4 className="font-medium text-white">Pipeline health check</h4>
                      ...
                      <div className="mt-3">
                        <Button size="sm" variant="outline" className="border-white/10" onClick={() => setActiveTab("scoring")}>
              ...
              <CardContent className="p-6">
                ...
                <div className="space-y-4">
                  <div className="p-4 bg-white/5 rounded-lg border border-white/5">
                    ...
                    <div className="text-2xl font-bold text-green-400">{scoreBuckets.hot}</div>
                  ...
                  <div className="p-4 bg-white/5 rounded-lg border border-white/5">
                    ...
                    <div className="text-2xl font-bold text-yellow-400">{scoreBuckets.warm}</div>
                  ...
                  <div className="p-4 bg-white/5 rounded-lg border border-white/5">
                    ...
                    <div className="text-2xl font-bold text-red-400">{scoreBuckets.cold}</div>
        ...
        {activeTab === "drafts" && (
          <Card className="glass-panel border-white/5 col-span-full">
            <CardContent className="p-6 flex gap-6 min-h-[500px] flex-col md:flex-row">
              <div className="w-full md:w-1/3 border-b md:border-b-0 md:border-r border-white/10 pb-4 md:pb-0 md:pr-6 space-y-3">
                ...
                {templates.map((t, i) => (
                  <div
                    ...
                    className={`p-3 rounded-lg border cursor-pointer transition-colors ${i === selectedTemplate ? "bg-primary/10 border-primary/30 text-primary" : "bg-black/20 border-white/5 text-muted-foreground hover:bg-white/5"}`}
                  >
                    {t.label}
                  </div>
                ))}
                ...
                <div className="pt-4 space-y-3 border-t border-white/10">
                  <div>
                    ...
                    <input
                      className="w-full bg-black/30 border border-white/10 rounded px-3 py-1.5 text-sm text-white placeholder:text-muted-foreground"
              ...
              <div className="flex-1 flex flex-col">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-sm text-white font-medium">{currentTemplate.subject}</span>
                </div>
                <div className="flex-1 bg-black/30 rounded-xl border border-white/5 p-6 font-mono text-sm text-gray-300 whitespace-pre-wrap leading-relaxed overflow-y-auto">
                  {draftBody}
                </div>
                ...
                {draftGenerated && (
                  <div className="mt-3 p-3 rounded-lg bg-green-500/10 border border-green-500/20 flex items-center gap-2 text-sm text-green-400">
              ...
              <CardContent className="p-6">
                ...
                <div className="space-y-4">
                  {[
                    ...
                    {
                      label: "Behaviour Score", weight: "40%", icon: TrendingUp, color: "text-accent",
                    ...
                    <div key={factor.label} className="p-4 rounded-xl border border-white/10 bg-black/20">
                      ...
                      <div className="grid grid-cols-2 gap-1.5">
                        {factor.breakdown.map(b => (
                          <div key={b.label} className="flex justify-between items-center text-xs bg-white/5 rounded px-2 py-1">
            ...
            <Card className="glass-panel border-white/5">
              <CardContent className="p-6">
                ...
                {leadsLoading ? (
                  ...
                  <div className="space-y-4">
                    {[
                      { label: "Hot (80–100)", count: scoreBuckets.hot, color: "bg-green-500", text: "text-green-400" },
                      { label: "Warm (50–79)", count: scoreBuckets.warm, color: "bg-yellow-500", text: "text-yellow-400" },
                      { label: "Cold (1–49)", count: scoreBuckets.cold, color: "bg-red-500", text: "text-red-400" },
                      { label: "Unscored", count: leads.filter(l => l.score == null).length, color: "bg-gray-500", text: "text-gray-400" },
                    ].map(bucket => (
                      <div key={bucket.label} className="p-3 bg-white/5 rounded-lg">
