    ...
    <Layout>
      <div className="flex flex-col gap-4 max-w-[1600px] mx-auto">
        ...
        <Card className="border-border p-4 flex items-center gap-3">
          <div className="w-10 h-8 rounded-full bg-indigo-500/15 flex items-center justify-center shrink-0">
        ...
        {actionState && (
          <Card className="border-border p-4 flex flex-col gap-3 bg-muted/10">
            ...
            <Textarea
              ...
              placeholder={actionState.mode === "reject" ? "Reason for rejection (required)…" : "Optional comment…"}
              className="text-sm min-h-[60px] bg-card"
              rows={2}
            />
            {actionState.mode === "reject" && !actionComment.trim() && (
              <div className="text-xs text-red-600">A comment is required to reject this request.</div>
            )}
