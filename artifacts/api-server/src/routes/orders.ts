...
router.get("/orders", async (req, res) => {
  try {
    ...
    const rawData = await db
      ...
      .where(eq(ordersTable.orgId, req.orgId!))
    ...
    const [countResult] = await db.select({ count: sql<number>`count(*)` }).from(ordersTable).where(eq(ordersTable.orgId, req.orgId!));
    res.json({
...
router.post("/orders", async (req, res) => {
  try {
    ...
    const [maxOrder] = await db.select({ maxNum: sql<string>`max(order_number)` }).from(ordersTable).where(eq(ordersTable.orgId, req.orgId!));
...
router.post("/orders/from-quote/:quoteId", async (req, res) => {
  try {
    ...
    const [quote] = await db.select().from(quotesTable).where(and(eq(quotesTable.id, quoteId), eq(quotesTable.orgId, req.orgId!)));
    ...
    const quoteItems = await db.select().from(quoteItemsTable).where(and(eq(quoteItemsTable.quoteId, quoteId), eq(quoteItemsTable.orgId, req.orgId!)));
...
router.get("/orders/:id", async (req, res) => {
  try {
    ...
    const [order] = await db
      ...
      .leftJoin(accountsTable, eq(ordersTable.accountId, accountsTable.id))
      .leftJoin(quotesTable, eq(ordersTable.quoteId, quotesTable.id))
      .where(and(eq(ordersTable.id, id), eq(ordersTable.orgId, req.orgId!)));
...
router.put("/orders/:id", async (req, res) => {
  try {
    ...
    const [existing] = await db.select().from(ordersTable).where(and(eq(ordersTable.id, id), eq(ordersTable.orgId, req.orgId!)));
...
router.delete("/orders/:id", async (req, res) => {
  try {
    ...
    if (!id) { res.status(400).json({ error: "Invalid order ID" }); return; }
    await db.delete(orderItemsTable).where(eq(orderItemsTable.orderId, id));
    await db.delete(ordersTable).where(and(eq(ordersTable.id, id), eq(ordersTable.orgId, req.orgId!)));
