...
router.get("/product-bundles", async (req, res) => {
  try {
    const bundles = await db.execute(sql`SELECT * FROM product_bundles ORDER BY name ASC`);
...
router.get("/product-bundles/:id", async (req, res) => {
  try {
    ...
    const bundles = await db.execute(sql`SELECT * FROM product_bundles WHERE id = ${id}`);
...
router.post("/product-bundles", async (req, res) => {
  try {
    ...
    const result = await db.execute(sql`
      INSERT INTO product_bundles (name, description, bundle_discount_pct, is_active)
      VALUES (${name}, ${description ?? null}, ${bundle_discount_pct}, ${is_active})
      RETURNING *
    `);
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      await db.execute(sql`
        INSERT INTO product_bundle_items (bundle_id, product_id, quantity, unit_price_override, discount_pct, sort_order)
        VALUES (${bundle.id}, ${item.product_id}, ${item.quantity ?? 1}, ${item.unit_price_override ?? null}, ${item.discount_pct ?? 0}, ${i})
      `);
    }
    // Return with items
    const fullItems = await db.execute(sql`
      ...
      WHERE pbi.bundle_id = ${bundle.id} ORDER BY pbi.sort_order
    `);
...
router.put("/product-bundles/:id", async (req, res) => {
  try {
    ...
    if (Array.isArray(items)) {
      await db.execute(sql`DELETE FROM product_bundle_items WHERE bundle_id = ${id}`);
      for (let i = 0; i < items.length; i++) {
        const item = items[i];
        await db.execute(sql`
          INSERT INTO product_bundle_items (bundle_id, product_id, quantity, unit_price_override, discount_pct, sort_order)
          VALUES (${id}, ${item.product_id}, ${item.quantity ?? 1}, ${item.unit_price_override ?? null}, ${item.discount_pct ?? 0}, ${i})
        `);
      }
    }
    const bundle = await db.execute(sql`SELECT * FROM product_bundles WHERE id = ${id}`);
    const fullItems = await db.execute(sql`
      SELECT pbi.*, p.name as product_name, p.unit_price as default_unit_price
      FROM product_bundle_items pbi JOIN products p ON p.id = pbi.product_id
      WHERE pbi.bundle_id = ${id} ORDER BY pbi.sort_order
    `);
...
router.delete("/product-bundles/:id", async (req, res) => {
  try {
    const id = parseInt(req.params.id);
    await db.execute(sql`DELETE FROM product_bundle_items WHERE bundle_id = ${id}`);
    await db.execute(sql`DELETE FROM product_bundles WHERE id = ${id}`);
