  ...
  switch (name) {
    ...
    case "get_pipeline_summary": {
      const rows = await db
        ...
        .from(opportunitiesTable)
        .where(and(eq(opportunitiesTable.orgId, orgId), sql`stage NOT IN ('closed_won', 'closed_lost')`))
    ...
    case "get_revenue_summary": {
      ...
      const conds = [eq(opportunitiesTable.orgId, orgId), eq(opportunitiesTable.stage, "closed_won")];
    ...
    case "get_workspace_overview": {
      const [
        ...
        db.select({ c: sql<number>`count(*)`, r: sql<number>`coalesce(sum(amount), 0)` })
          .from(opportunitiesTable).where(and(eq(opportunitiesTable.orgId, orgId), eq(opportunitiesTable.stage, "closed_won"))),
      ]);
      return {
        counts: {
...
router.post("/ai/summary", async (req, res) => {
  if (!requireAuth(req, res)) return;
  const orgId = getSessionUser(req)!.orgId;
  ...
  try {
    ...
    switch (entityType) {
      ...
      case "contacts": {
        ...
        const withAccount = await db.select({ count: sql<number>`count(*)` }).from(contactsTable).where(and(eq(contactsTable.orgId, orgId), sql`account_id is not null`));
