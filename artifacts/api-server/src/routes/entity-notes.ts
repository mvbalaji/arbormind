...
router.get("/notes", async (req, res) => {
  try {
    ...
    const rows = await db.select().from(entityNotesTable)
      .where(and(eq(entityNotesTable.entityType, entity), eq(entityNotesTable.entityId, idNum), eq(entityNotesTable.orgId, req.orgId!)))
...
router.patch("/notes/:id", async (req, res) => {
  try {
    ...
    if (id == null) return;
    const [existing] = await db.select().from(entityNotesTable).where(and(eq(entityNotesTable.id, id), eq(entityNotesTable.orgId, req.orgId!)));
    ...
    const [updated] = await db.update(entityNotesTable).set(updates).where(and(eq(entityNotesTable.id, id), eq(entityNotesTable.orgId, req.orgId!))).returning();
...
router.delete("/notes/:id", async (req, res) => {
  try {
    ...
    const [existing] = await db.select().from(entityNotesTable).where(and(eq(entityNotesTable.id, id), eq(entityNotesTable.orgId, req.orgId!)));
    if (!existing) { res.status(404).json({ error: "Note not found" }); return; }
    await db.delete(entityNotesTable).where(and(eq(entityNotesTable.id, id), eq(entityNotesTable.orgId, req.orgId!)));
