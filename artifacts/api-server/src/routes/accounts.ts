...
async function enrichAccounts(data: Array<{ id: number; [key: string]: unknown }>, orgId: number) {
  ...
  const [contactCounts, dealCounts] = await Promise.all([
    db.select({ accountId: contactsTable.accountId, count: sql<number>`count(*)` })
      .from(contactsTable)
      .where(and(inArray(contactsTable.accountId, accountIds), eq(contactsTable.orgId, orgId)))
      .groupBy(contactsTable.accountId),
    db.select({ accountId: opportunitiesTable.accountId, count: sql<number>`count(*)` })
      .from(opportunitiesTable)
      .where(and(inArray(opportunitiesTable.accountId, accountIds), eq(opportunitiesTable.orgId, orgId)))
...
router.get("/accounts/:id", async (req, res) => {
  try {
    ...
    const [contactCounts, dealCounts] = await Promise.all([
      db.select({ count: sql<number>`count(*)` }).from(contactsTable).where(and(eq(contactsTable.accountId, account.id), eq(contactsTable.orgId, orgId))),
      db.select({ count: sql<number>`count(*)` }).from(opportunitiesTable).where(and(eq(opportunitiesTable.accountId, account.id), eq(opportunitiesTable.orgId, orgId))),
    ]);
