...
router.get("/accounts", async (req, res) => {
  try {
    ...
    const conditions = [eq(accountsTable.orgId, req.orgId!)];
    ...
    const [countResult] = await db.select({ count: sql<number>`count(*)` }).from(accountsTable).where(and(...conditions));
...
router.post("/accounts", async (req, res) => {
  try {
    const payload: Partial<InsertAccount> = { orgId: req.orgId! };
...
router.get("/accounts/:id", async (req, res) => {
  try {
    ...
    const [account] = await db
      ...
      .from(accountsTable)
      .leftJoin(usersTable, eq(accountsTable.ownerId, usersTable.id))
      .where(and(eq(accountsTable.id, id), eq(accountsTable.orgId, req.orgId!)));
...
router.delete("/accounts/:id", async (req, res) => {
  try {
    const id = parseId(req.params.id);
    if (!id) { res.status(400).json({ error: "Invalid account ID" }); return; }
    await db.delete(accountsTable).where(and(eq(accountsTable.id, id), eq(accountsTable.orgId, req.orgId!)));
...
router.get("/accounts/:id/contacts", async (req, res) => {
  try {
    ...
    const data = await db
      .select()
      .from(contactsTable)
      .where(and(eq(contactsTable.accountId, accountId), eq(contactsTable.orgId, req.orgId!)))
...
router.get("/accounts/:id/opportunities", async (req, res) => {
  try {
    ...
    const data = await db
      .select()
      .from(opportunitiesTable)
      .where(and(eq(opportunitiesTable.accountId, accountId), eq(opportunitiesTable.orgId, req.orgId!)))
...
router.get("/accounts/:id/activities", async (req, res) => {
  try {
    ...
    const data = await db
      ...
      .where(and(eq(activitiesTable.accountId, accountId), eq(activitiesTable.orgId, req.orgId!)))
...
router.get("/accounts/:id/quotes", async (req, res) => {
  try {
    ...
    const data = await db
      .select()
      .from(quotesTable)
      .where(and(eq(quotesTable.accountId, accountId), eq(quotesTable.orgId, req.orgId!)))
...
router.get("/accounts/:id/cases", async (req, res) => {
  try {
    ...
    const data = await db
      .select()
      .from(casesTable)
      .where(and(eq(casesTable.accountId, accountId), eq(casesTable.orgId, req.orgId!)))
